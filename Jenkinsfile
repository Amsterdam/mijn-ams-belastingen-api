...
String PROJ_NAME = "mijn-ams-belastingen-api"
        ...
        stage('Push acceptance image') {
            tryStep "image tagging", {
                docker.withRegistry("${DOCKER_REGISTRY_HOST}",'docker_registry_auth') {
                    ...
                    retagAndPush("mijnams/${PROJ_NAME}", "acceptance")
                }
            }
        }
    }
            ...
            tryStep "deployment", {
                build job: 'Subtask_Openstack_Playbook',
                    parameters: [
                        ...
                        [$class: 'StringParameterValue', name: 'PLAYBOOKPARAMS', value: "-e cmdb_id=app_mijn-belastingen"]
                    ]
            }
        }
    }
    ...
    node {
        stage('Push production image') {
            tryStep "image tagging", {
                docker.withRegistry("${DOCKER_REGISTRY_HOST}",'docker_registry_auth') {
                    docker.image("mijnams/${PROJ_NAME}:${env.BUILD_NUMBER}").pull()
                    retagAndPush("mijnams/${PROJ_NAME}", "production")
                    retagAndPush("mijnams/${PROJ_NAME}", "latest")
                }
            }
        }
    }
